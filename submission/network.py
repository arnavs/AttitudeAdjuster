...
def get_regret_matching_strategy(net, infoset_vec, legal_mask, device='cpu'):
    """
    Convert an advantage network output into a strategy via regret matching.

    This is the correct inference rule for the Deep CFR value / advantage nets.
    """
    with torch.no_grad():
...
def get_policy_distribution(net, infoset_vec, legal_mask, device='cpu'):
    """
    Convert a trained strategy network output into a masked policy distribution.

    The final Deep CFR strategy networks are trained with cross-entropy on
    target action probabilities, so they should be queried with a softmax, not
...
def get_strategy(net, infoset_vec, legal_mask, device='cpu'):
    """
    Backward-compatible alias for regret-matching inference.
    """
    return get_regret_matching_strategy(net, infoset_vec, legal_mask, device)
