...
class PlayerAgent(Agent):
    ...
    def _thompson_action(self, observation):
        ...
        # Probe bet: small raise when we'd normally check (~8%), need some equity
        if valid_actions[self.action_types.CHECK.value] and valid_actions[self.action_types.RAISE.value]:
            if win_rate > 0.35 and np.random.random() < 0.08:
                return self.action_types.RAISE.value, min_raise, 0, 0

        if valid_actions[self.action_types.CHECK.value]:
            # Noisy raise: occasional raise with mediocre holdings (~5%)
            if 0.45 < win_rate <= 0.65 and valid_actions[self.action_types.RAISE.value]:
                if np.random.random() < 0.05:
                    raise_amount = np.random.randint(min_raise, min(min_raise + 4, max_raise + 1))
                    return self.action_types.RAISE.value, raise_amount, 0, 0
    ...
    def _update_on_action(self, observation):
        """Bayesian update on opponent range from their betting action (turn/river only)."""
        street = observation["street"]
        if street < 2 or self.opp_pairs is None:
            return
        action_key = f"bet_{street}"
        if action_key in self.zeroed_streets:
            return
        self.zeroed_streets.add(action_key)
        ...
        # Detect a genuine raise: opponent bet more than needed to match us
        opp_raised = opp_bet > my_bet and opp_bet > my_bet + 1  # more than just completing a blind

        # Rank each candidate pair on the current board (lower treys rank = stronger)
        board_treys = [PokerEnv.int_to_card(c) for c in community]
        if len(community) == 5:
            ranks = np.array([
                self.evaluator.evaluate([PokerEnv.int_to_card(h1), PokerEnv.int_to_card(h2)], board_treys)
                for h1, h2 in self.opp_pairs
            ], dtype=np.float64)
        elif len(community) == 4:
            # Average rank over possible river cards
            dead = set(observation["my_cards"]) | set(observation["my_discarded_cards"]) | set(observation["opp_discarded_cards"]) | set(community)
            dead.discard(-1)
            pool = [c for c in range(27) if c not in dead]
            ranks = np.zeros(len(self.opp_pairs), dtype=np.float64)
            for idx, (h1, h2) in enumerate(self.opp_pairs):
                opp_treys = [PokerEnv.int_to_card(h1), PokerEnv.int_to_card(h2)]
                river_pool = [c for c in pool if c != h1 and c != h2]
                r = 0.0
                for rv in river_pool:
                    board5 = board_treys + [PokerEnv.int_to_card(rv)]
                    r += self.evaluator.evaluate(opp_treys, board5)
                ranks[idx] = r / len(river_pool)
        else:
            return  # flop: skip, too expensive

        # Convert ranks to 0-1 strength (rank-based percentile within the candidate set)
        strengths = 1.0 - (ranks - ranks.min()) / max(ranks.max() - ranks.min(), 1e-9)

        if opp_raised:
            # Scale strength of update by raise size relative to pot
            raise_size = opp_bet - my_bet
            aggression = min(raise_size / max(pot, 1), 2.0)
            TEMP = 2.0 + 2.0 * aggression  # 2-6 range
            log_w = TEMP * strengths
        else:
            # Checked or just called → downweight strong opponent hands
            TEMP = 2.0
            log_w = -TEMP * strengths

        log_w -= log_w.max()
        self.opp_weights *= np.exp(log_w)
        s = self.opp_weights.sum()
        if s > 0:
            self.opp_weights /= s
    ...
    def _act_turn(self, observation):
        ...
        self._update_on_action(observation)
        return self._thompson_action(observation)
